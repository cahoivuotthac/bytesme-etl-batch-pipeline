...
website_name = "breadtalk"
web_config = config["websites"]
logger.debug(f"breadtalk config: {web_config}")

extractor = ProductExtractor(web_config, website_name)
...
all_products = []
for url in category_urls[website_name]:
...
df.to_csv('data/test/breaktalk_products.csv', index=False, encoding='utf-8')
